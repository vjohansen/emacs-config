...
;; Copyright (C) 2001, 2002 Hans-J?gen Ficker
...
;;   the delimiter between the signatures (thanks to Andreas B?ching
;;   <crunchy@tzi.de>)
...
list of strings.
The files are searched in `randomsig-dir', if they don't have absolute path
s.
...
(defun randomsig-replace-signature-in-signature-files (signature)
  ...
  (f sig start (- sig start f start))
  (f sig end (- sig end f start))
  ;; old signature
  (old sig (randomsig-signature-at-point)))
    (set-buffer (find-file-noselect (expand-file-name fname randomsig-dir)))

    (if (not (string= old sig (buffer-substring f sig start f sig end)))
 (error "Signature file has changed"))
    (delete-region f sig start f sig end)
    (goto-char f sig start)
    (insert signature)
    (save-buffer))
...
\\{randomsig-select-mode-map}"
  ...
  (define-key randomsig-select-mode-map (kbd "RET") 'randomsig-select-repla
ce)
